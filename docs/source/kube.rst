Scaling [tml-multi-agenticai-iot-3f10-3315-ml_agenticai] With Kubernetes
===========================
...
Based on your TML solution [tml-multi-agenticai-iot-3f10-3315-ml_agenticai] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresstml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml -f ollama.yml

tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml
------------------------

.. important::
   Copy and Paste this YAML file: tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/Bobbie101/raspberrypi/tree/main/tml-airflow/dags/tml-solutions/tml-multi-agenticai-iot-3f10-3315/ymls/tml-multi-agenticai-iot-3f10-3315-ml_agenticai
...
.. code-block:: YAML
   ...
   ################# tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml
     ...
     kind: Deployment
     metadata:
       name: tml-multi-agenticai-iot-3f10-3315-ml_agenticai
     spec:
       selector:
         matchLabels:
           app: tml-multi-agenticai-iot-3f10-3315-ml_agenticai
       ...
       template:
         ...
         spec:
           containers:
           - name: tml-multi-agenticai-iot-3f10-3315-ml_agenticai
             image: Bobbie101/tml-multi-agenticai-iot-3f10-3315-ml_agenticai-arm64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'tml-multi-agenticai-iot-3f10-3315'                              
             - name: SOLUTIONNAME
               value: 'tml-multi-agenticai-iot-3f10-3315-ml_agenticai'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_ml_agenticai_dag-tml-multi-agenticai-iot-3f10-3315'
             - name: GITUSERNAME
               value: 'Bobbie101'
             - name: GITREPOURL
               value: 'https://github.com/Bobbie101/raspberrypi.git'
             ...
             - name: step9bollamacontainername
               value: 'maadsdocker/tml-privategpt-with-cpu-amd64-llama3-tools'
             ...
             - name: step9bagents_topic_prompt
               value: 'iot-preprocess<<- Your task is to identify unusual temperature changes, environmental anomalies, or noteworthy sensor patterns that could indicate shifts in underwater conditions.         The json data come from marine sensors.        The hp field shows processed values for the measured variable using process types such as avg, trend, or anomprob. The monitored sensor or entity is in the uid field.         here is the json data:              <<data>>         INSTRUCTIONS:         1. Examine each number in the json array         2. Provide a brief analysis of the results                  FORMAT YOUR RESPONSE:         - Filtered results: [list the qualifying numbers with their "uid" fields]         - Count of qualifying numbers: [number]         - Analysis: [brief explanation of what the filter revealed]                  Be precise and concise in your response.->>        iot-ml-prediction-results-output<<-You are a precise marine data analysis assistant. Your task is to filter and analyze numeric data based on specified criteria.        TASK: Filter numbers from the given json array using the threshold: greater than 90        Input JSON arrary:              <<data>>         INSTRUCTIONS:         1. Examine each number in the json array         2. Apply the filter condition: number > 90         3. Return only numbers that meet the criteria with their "uid" fields         4. If no numbers meet the criteria, explicitly state this         5. Provide a brief analysis of the results                  FORMAT YOUR RESPONSE:         - Filtered results: [list the qualifying numbers with their "uid" fields]         - Count of qualifying numbers: [number]         - Analysis: [brief explanation of what the filter revealed]                  Be precise and concise in your response.'
             - name: step9bteamlead_topic
               value: 'team-lead-responses'
             - name: step9bteamleadprompt
               value: 'Analyze the marine sensor monitoring records produced by individual agents. Review all data fields to determine whether any readings indicate significant environmental concern.           Focus on the following criteria:          1. Each record contains a unique identifier stored in the field "uid".          2. Examine the anomaly or prediction values stored in the hp field.          3. Categorize the values as follows:           - Low: 0% to 50%           - Medium: 51% to 75%           - High: 76% to 89%           - Urgent: 90% to 100%          Tasks:         - Identify and highlight sensors or monitored entities (by their "uid") with urgent values (≥ 90%).         - For each flagged result, explain why it may require immediate review.         - Only include records that meet the urgent threshold unless brief context is needed.         - State clearly whether the identified issue is urgent.         - Do not use or generate code.'
             ...
             - name: step1solutiontitle # STEP 1 solutiontitle field can be adjusted here. 
               value: 'Marine Sensor AI Monitoring Solution'                              
             - name: step1description # STEP 1 description field can be adjusted here. 
               value: 'A real-time marine sensor monitoring solution designed to detect temperature increases, shifts in underwater environmental conditions, and unusual biological activity for ocean observation and early anomaly detection.'                                          
             - name: KUBEBROKERHOST
   ...
   ---
     ...
     kind: Service
     metadata:
       name: tml-multi-agenticai-iot-3f10-3315-ml_agenticai-visualization-service
       labels:
         app: tml-multi-agenticai-iot-3f10-3315-ml_agenticai-visualization-service
     spec:
       type: ClusterIP
       ports:
      ...
      spec:
        ...
        template:
          ...
          spec:
            containers:
            - name: ollama
              image: maadsdocker/tml-privategpt-with-cpu-amd64-llama3-tools # IF you DO NOT have NVIDIA GPU then CPU will be used
              imagePullPolicy: IfNotPresent  # You can also use Always, Never
              env:
              - name: NVIDIA_VISIBLE_DEVICES
                value: all
              ...
              - name: supervisorprompt
                value: "You are a team supervisor analyzing marine monitoring results and recommending whether an alert should be sent.          You manage a send email expert and an average expert.         For send email, use send_email agent.         For average, use average agent.       INSTRUCTIONS:       1.Analyze the Team Lead assessment and determine the proper action:       - If sensors are marked urgent or values exceed 90%, select "send_email".       - If no urgent conditions are found, then no action is needed."
...
.. code-block::
   ...
   kubectl port-forward deployment/tml-multi-agenticai-iot-3f10-3315-ml_agenticai 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/iot-failure-machinelearning.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-tml-multi-agenticai-iot-3f10-3315-ml_agenticai.yml
...
.. code-block::
   ...
   http://tml.tss/viz/iot-failure-machinelearning.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
